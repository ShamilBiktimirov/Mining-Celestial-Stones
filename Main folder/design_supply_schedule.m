function [C_opt, H_opt] = design_supply_schedule(M_storage_in, Destination, M_storage, M_storage_opt, H, H_opt, consts)
...
% M_storage_in - initial mass of resources at the storages
% Destination - index of destination
% M_storage - current mass of resources at the storage
% M_storage_opt - mass of resouurces in the storage by the end of the trip
% H: detailed history of current schedule: [transfer destination, date, duration, dV]
% H_opt: new optimal schedule history
    ...
    elseif T >= consts.T_f
        ...
        for A_n = options(A, consts) % all possible destinations to visit if on Mars and back trip to Mars if on mining site
