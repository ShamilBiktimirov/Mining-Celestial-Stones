...
consts.T_0 = juliandate(datetime(2050,1,1,0,0,0));         % start of schedule (JD): 1/1/2030
consts.T_f = 50*365;            % length of schedule (days)
...
consts.dT_t = 1/24;                % transfer search increment (days)
consts.dV_max = 100;             % maximum dV capability per transfer
...
oes_ep = [1.524*AU, 0.094, 1.851, 49.579, 336.041, 355.453, 2451545.0, 42828, 3398.5, 500;...       % Mars
          1.000*AU, 0.0167, 0.000, -11.261, 102.947, 100.464, 2451545.0, 398600.44158, 6317, 500];  % Earth
...
% preprocessing
tic;
consts.transfers = preprocess(oes_ep, consts);
toc;
